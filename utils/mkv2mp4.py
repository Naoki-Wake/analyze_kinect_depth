        ...
        while True:
            try:
                ...
                if (capture.color is not None) and (
                    ...
                    #frame_d = colorize(capture.depth, (None, 5000))
                    # depth_stack.append(capture.depth)
                    ...
                    if H != H_d or W != W_d:
                        ...
                        raise RuntimeError
                    frame = cv2.resize(
                        frame, (W, H), interpolation=cv2.cv2.INTER_NEAREST)
                    frame_d = cv2.resize(
                        frame_d, (W_d, H_d), interpolation=cv2.cv2.INTER_NEAREST)
