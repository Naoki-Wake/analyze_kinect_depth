...
class task_daemon:
    def __init__(
        ...
        self.task = task
        with open(os.path.join(os.getcwd(), 'utils', 'task_models', self.task + '.json')) as f:
            self.taskmodel_json = json.load(f)
        self.verbal_input = verbal_input
        self.fp_mp4 = fp_mp4
        self.fp_depth_npy = fp_depth_npy
        self.segment_timings_frame = segment_timings_frame
        self.output_dir = os.path.join(
            output_dir, 'daemon', 'frame_{}-{}_'.format(
                segment_timings_frame[0], segment_timings_frame[1]) + task)
        self.total_count = self.segment_timings_frame[1] - \
            self.segment_timings_frame[0]
        self.id = 'frame_{}-{}_'.format(segment_timings_frame[0], segment_timings_frame[1]) + task
        # create dir
        ...
        if ret:
            ...
            cap.set(cv2.CAP_PROP_POS_FRAMES, self.segment_timings_frame[0] - 1)
        ...
        self.last_frame_depth = depth_array[self.segment_timings_frame[1]]
        ...
        if hand_laterality == 'unknown':  # recognizie hand laterality
            # find object
            self.object_name, self.object_attribute = self.get_object_name()
    ...
    def extract_hands_with_laterality(self):
        ...
        if len(handloc_first) > 0:
            ...
            # frame_crop_first_frame_depth = self.first_frame_depth[
            #     handloc_first['top']:handloc_first['bottom'],
            #     handloc_first['left']:handloc_first['right']]
            # fp_first_frame_hand_depth = os.path.join(
            #     self.output_dir, 'first_frame_hand_depth.npy')
            # np.save(fp_first_frame_hand_depth, frame_crop_first_frame_depth)
    ...
    def get_object_name(self):
        object_name, object_attribute = task_recognizer.run_object_recognition(
            self.verbal_input)
        return object_name, object_attribute
    ...
    def set_skillparameters(self):
        ...
        for param_name in skill_params:
            ...
            if "detach_direction" in param_name or "approach_direction" in param_name or "depart_direction" in param_name:
                if self.taskmodel_json[param_name]["value"] is None:
            ...
            if "rotation_axis" in param_name:
                if self.taskmodel_json[param_name]["value"] is None:
                    ...
                    if rotation_axis == [0, 0, 1]:
                        xi = loc_xyz_list[:, 0]
                        yi = loc_xyz_list[:, 1]
                        a, b, r = trajection_fitting.circle_fitting(xi, yi)
